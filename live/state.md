...
- **Next session: start here**
  1. Find hPanel CDN/Security bot-protection setting for lelaboratoire.ma, lower/disable it, retest in Edge browser specifically (was the last browser confirmed still failing after other fixes attempted).
  2. Run `npm run build:standalone` locally in `app/` to confirm the next/image changes build clean, then commit + push (Hostinger auto-builds on git push per its Node.js deployment config) to actually ship the PageSpeed fixes above.
  3. After deploy, re-run PageSpeed to confirm: `/images/*` cache-control now present, LCP/TBT actually measurable (previous run got NO_LCP — suspected same bot-challenge blocking PageSpeed's crawler too, should resolve once item 1 is fixed).
  4. More PageSpeed findings may still be queued from the user — ask if they have more stats to hand over before continuing new fixes.
